...
for t in nextjs nestjs trigger pentest kotlin; do log "deps: $t"; runuser -u player -- bash -c "export PATH=/opt/kaputt/bin:\$PATH; cd /opt/kaputt && WG_TRACK=$t timeout 1200 wg start >/dev/null 2>&1 && echo '  ok' || echo '  (non-zero)'"; done
