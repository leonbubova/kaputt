## 1
Edit `data/stock.json`, then `curl localhost:3100/api/stock` (fresh) vs `curl -s localhost:3100/stock | grep tauwurm` (stale). The API is fine; the page's data loading is the problem.
## 2
