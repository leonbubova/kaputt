...
## 3
`docker logs wg-web 2>&1 | grep 'GET /checkout' | grep ' 404 ' | grep -o 'rid=.*' | cut -d= -f2- > ~/.k8s-wargame/docker/33-find-request/answer.txt` (mind the error-log line — it has no `rid=`).
