...
## 3
`cd ~/.k8s-wargame/docker/11-build-image && docker build -t wg-site:1 .` — the `.` is the build context (the directory `COPY` reads from). Test: `docker run --rm -p 18003:80 wg-site:1`.
