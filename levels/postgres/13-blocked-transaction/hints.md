## 1
An UPDATE takes a row lock and holds it until its transaction commits or rolls back; another writer on the same row simply waits — forever, if the holder never finishes. Reads don't need that lock, which is why they still work. Who is waiting on whom? `select pid, state, wait_event_type, application_name, left(query, 60) from pg_stat_activity where datname = 'wg';`
## 2
