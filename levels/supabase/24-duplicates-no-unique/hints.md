## 1
Only the database can guarantee uniqueness — an app-side check races with itself, a `unique` constraint doesn't. Two jobs, in order: remove the duplicates, then make the database enforce uniqueness. Find them: `select email, count(*) from subscribers group by 1 having count(*) > 1;`
## 2
