...
c="$WGH/26-subchart-condition/shop"
...
helm template shop "$c" -f "$WGH/26-subchart-condition/values-prod.yaml" 2>/dev/null | grep -q 'name: shop-cache' || fail "with -f values-prod.yaml the cache is not rendered"
