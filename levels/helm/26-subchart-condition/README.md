...
Work dir: `~/.k8s-wargame/helm/26-subchart-condition/`. Goal: `helm template shop ./shop` contains no `shop-cache`, with `-f values-prod.yaml` it does; release `shop` deployed, `shop` and `shop-cache` available.
