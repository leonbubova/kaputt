...
Chart copy: `~/.k8s-wargame/helm/32-secret-in-values/shop`. Goal: neither `helm get values --all` nor `helm get manifest` contain the password (plain or base64), pod still gets `DB_PASSWORD` from a Secret and is available.
