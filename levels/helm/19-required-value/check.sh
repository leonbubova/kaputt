...
helm template shop "$c" -f "$WGH/19-required-value/values-prod.yaml" >/dev/null 2>&1 || fail "helm template shop ./shop -f values-prod.yaml fails — prod must still render"
