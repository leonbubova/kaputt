...
Work dir: `~/.k8s-wargame/helm/19-required-value/`. Goal: `helm template shop ./shop` fails with that message, `helm template shop ./shop -f values-prod.yaml` renders, release `shop` deployed with 2 replicas and `domain=shop.example.com`.
