...
Chart copy: `~/.k8s-wargame/helm/13-if-block/web`. Goal: page at `http://web.wg-helm.svc/` shows `debug=on`; `helm template web ./web` (chart defaults) contains no `debug` line.
