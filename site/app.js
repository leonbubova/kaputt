...
fetch('levels.json').then(r=>r.json()).then(d=>{
  ...
  d.tracks.forEach(t=>{
    ...
    const lis=t.levels.map(l=>`<li><span class="ln">${l.n}</span><span><span class="sl">${l.slug}</span><br>${l.title.replace(/</g,'&lt;')}</span></li>`).join('');
    el.innerHTML=`<h3>${t.track}<span class="n">${t.count}</span></h3><div class="bl">${t.blurb}</div>`+
      `<details><summary>show ${t.count} levels</summary><ol>${lis}</ol></details>`;
    grid.appendChild(el);
  });
